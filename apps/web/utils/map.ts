...
export const boundsToNaverBounds = (bounds: Bounds) => {
  return new window.naver.maps.LatLngBounds(new window.naver.maps.LatLng(bounds.min.y, bounds.min.x), new window.naver.maps.LatLng(bounds.max.y, bounds.max.x))
}
